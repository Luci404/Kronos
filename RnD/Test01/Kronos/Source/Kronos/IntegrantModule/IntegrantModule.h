...
namespace Kronos
{
    ...
    {
    public:
        friend std::hash<IntegrantHandle<T>>;
...
template <std::derived_from<Kronos::IIntegrant> T>
struct std::hash<Kronos::IntegrantHandle<T>>
{
    std::size_t operator()(const Kronos::IntegrantHandle<T> &integrantHandle) const
    {
